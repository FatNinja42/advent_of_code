...
class Cube {
    ...
    public Cube(List<Coordinate> cube) {
        for (Coordinate coordinate : cube) {
        ...
        for (int x = min[0], maxX = this.max[0]; x <= maxX; x++) {
            for (int y = min[1], maxY = this.max[1]; y <= maxY; y++) {
                for (int z = min[2], maxZ = this.max[2]; z <= maxZ; z++) {
                    for (int w = min[3], maxW = this.max[3]; w <= maxW; w++) {
                        final Coordinate coordinate = new Coordinate(x, y, z, w);
        ...
        for (int x = position[0] - 1; x <= position[0] + 1; x++) {
            for (int y = position[1] - 1; y <= position[1] + 1; y++) {
                for (int z = position[2] - 1; z <= position[2] + 1; z++) {
                    for (int w = position[3] - 1; w <= position[3] + 1; w++) {
                        if (x == position[0]
                                && y == position[1]
                                && z == position[2]
                                && w == position[3]
                        ...
                        count += this.cube.contains(new Coordinate(x, y, z, w)) ? 1 : 0;
                    }
                }
            }
        }
